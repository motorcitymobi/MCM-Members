...
class MCM_Members_Search_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// echo '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" ><input type="hidden" value="" name="s" /><input type="hidden" value="fuck" name="post_type" />';
		echo '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" ><input type="hidden" value="" name="s" /><input type="hidden" value="members" name="post_type" />';
		...
		foreach ( $members_taxonomies as $tax => $data ) {
			...
			$terms = get_terms( $tax, array( 'orderby' => 'name', 'order' => 'ASC', 'number' => 200, 'hierarchical' => true ) );
			if ( empty( $terms ) )
				continue;
